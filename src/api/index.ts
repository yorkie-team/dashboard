...
export async function listDocumentHistories(
  ...
  try {
    ...
    const getSnapshotTree = async (serverSeq: string) => {
      ...
      document.applySnapshot(Long.fromString(serverSeq), snapshotMeta.snapshot);
      return document.getValueByPath('$.text') as any;
    };

    const getChangesTree = async (serverSeq: string) => {
